<script setup lang="ts">
	...
	watch(
		...
		async (newUserData) => {
			...
			userParticipation.value = await apiCall.getUserParticipation(
				userId.value,
				challengeId,
			);
	...
	onMounted(async () => {
		...
		participationsData.value =
			await apiCall.getParticipationsByChallengeId(challengeId);
		if (
			participationsData.value == null ||
			participationsData.value.length == 0
		) {
